"""
API service for fetching theme park data from queue-times.com
"""
import asyncio
import json

from ..models.theme_park import ThemePark
from ..models.theme_park_list import ThemeParkList
...
class QueueTimesAPI:
    ...
    async def update_parks(self):
        """
        Update all selected parks and return the data.
        
        Returns:
            Park list data for display
        """
        if hasattr(self.park_list, 'selected_parks') and self.park_list.selected_parks:
            await self.update_selected_parks()
        elif self.park_list and self.park_list.current_park.is_valid():
            await self.update_current_park()
